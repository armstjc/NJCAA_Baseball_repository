...
def getNjcaaGamelogs(teams_df: pd.DataFrame()):
    ...
    for i in tqdm(range(count, player_count)):
        ...
        try:
            ...
            gamelog_batting_df = pd.merge(gamelog_batting_df, gamelog_ex_batting_df,on=['date', 'location', 'opponent', 'result', 'score', 'team_score', 'opp_score', 'game_score_diff', 'game_win_loss', 'game_id'])
            ...
            if len(gamelog_batting_df) > 0:
                gamelog_batting_df.to_csv(
                    f'player_stats/batting/{game_season}_{game_player_id}.csv', index=False)
        ...
        try:    
            ...
            if len(gamelog_pitching_df) > 0:
                gamelog_pitching_df.to_csv(
                    f'player_stats/pitching/{game_season}_{game_player_id}.csv', index=False)
        ...
        try:
            ...
            for k in fielding_table.find_all('tr'):
                ...
                if len(row) < 2:
                    ...
                    game_fielding_RCS_PCT = str(
                        row[11].text.strip()).replace('-', '')
