...
def getNjcaaGamelogs(teams_df: pd.DataFrame()):
    ...
    for i in tqdm(range(count, player_count)):
        ...
        # TODO: Figure out what on earth this stat is for.
        game_batting_HDP = None
        game_batting_GO = None  # Ground Out
        game_batting_FO = None  # Fly Out
        game_batting_GO_FO = None  # Ground Out/Fly Out ratio
        # Plate Apperances (For some reason, this is in Extedned Hitting).
        game_batting_PA = None
        # Pitching
        ...
        # TODO: Figure out what on earth this stat is for.
        game_fielding_RCS = None
        # TODO: Figure out what on earth this stat is for.
        game_fielding_RCS_PCT = None
        ...
        try:
            ...
            for k in batting_table.find_all("tr"):
                ...
                if len(row) < 2:
                    ...
                    row_df = pd.DataFrame(
                        {
                            ...
                            "result": game_result,
                            "score": game_score,
                            "team_score": game_team_score,
                            "opp_score": game_opp_score,
                            "game_score_diff": game_score_diff,
                            "game_win_loss": game_win_loss,
                            "game_id": game_id,
                            "AB": game_batting_AB,
                            "R": game_batting_R,
                            "H": game_batting_H,
                    ...
                    gamelog_batting_df = pd.concat(
                        [gamelog_batting_df, row_df], ignore_index=True
                    )
                    del row_df
                    ...
                    row_df = pd.DataFrame(
                        {
                            ...
                            "game_win_loss": game_win_loss,
                            "game_id": game_id,
                            "HBP": game_batting_HBP,
                            "SF": game_batting_SF,
                            "SH": game_batting_SH,
                            "TB": game_batting_TB,
                            "XBH": game_batting_XBH,
                            "HDP": game_batting_HDP,
                            "GO": game_batting_GO,
                            "FO": game_batting_FO,
                            "GO/FO": game_batting_GO_FO,
                            "PA": game_batting_PA,
                        },
                        index=[0],
                    )
                    gamelog_ex_batting_df = pd.concat(
                        [gamelog_ex_batting_df, row_df], ignore_index=True
                    )
                    del row_df

            # print(gamelog_ex_batting_df)
            gamelog_batting_df = pd.merge(
                gamelog_batting_df,
                gamelog_ex_batting_df,
                on=[
                    "date",
                    "location",
                    "opponent",
            ...
            gamelog_batting_df = gamelog_batting_df.dropna(subset=["game_id"])
            if len(gamelog_batting_df) > 0:
                gamelog_batting_df.to_csv(
                    f"player_stats/batting/{game_season}_{game_player_id}.csv",
                    index=False,
                )
            del (
                gamelog_batting_df,
            ...
            print("could not get batting stats for this player")
        ...
        try:
            ...
            for k in pitching_table.find_all("tr"):
                ...
                if len(row) < 2:
                    ...
                    gamelog_pitching_df = pd.concat(
                        [row_df, gamelog_pitching_df], ignore_index=True
                    )
                    del row_df
            ...
            gamelog_pitching_df = gamelog_pitching_df.dropna(subset=["game_id"])
            if len(gamelog_pitching_df) > 0:
                gamelog_pitching_df.to_csv(
                    f"player_stats/pitching/{game_season}_{game_player_id}.csv",
                    index=False,
                )
            del gamelog_pitching_df, pitching_table
        except:
            print("could not get pitching stats.")
        ...
        try:
                    ...
                    row_df = pd.DataFrame(
                        {
                            ...
                            "A": game_fielding_A,
                            "E": game_fielding_E,
                            "FPCT": game_fielding_FPCT,
                            "DP": game_fielding_DP,
                            "SBA": game_fielding_SBA,
                            "RCS": game_fielding_RCS,
                            "RCS_PCT": game_fielding_RCS_PCT,
                            "PB": game_fielding_PB,
                            "CI": game_fielding_CI,
                        },
                        index=[0],
                    )
                    gamelog_fielding_df = pd.concat(
                        [gamelog_fielding_df, row_df], ignore_index=True
                    )
                    del row_df
            ...
            gamelog_fielding_df = gamelog_fielding_df.dropna(subset=["game_id"])
            if len(gamelog_fielding_df) > 0:
                gamelog_fielding_df.to_csv(
                    f"player_stats/fielding/{game_season}_{game_player_id}.csv",
                    index=False,
                )
        except:
